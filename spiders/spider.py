...
class GumSpider(scrapy.Spider):
    ...
    #tab_id_client=[]
    start_urls=list(pd.read_csv("gumtree_url04_10.csv")['url'])
#    start_urls= ['https://www.gumtree.co.za/a-cars-bakkies/somerset-west/ford-ranger-3-2-xlt-4x4-d-c-manual-fsh/1003842224800910174326509',

    '''    
    def parse(self, response):
        base_url="https://www.gumtree.co.za"
        liste=response.xpath("//a[@class='href-link tile-title-text']/@href").extract()
        ...
        for announce in liste:
            ...
            if "status=adInactive" in liste:
                ...
                yield scrapy.Request(url=announce,callback=self.parse_details,dont_filter=True)
    ...
    #        print("details -------------------------------")
            ...
            try:
               ...
               # prix=response.xpath("//div[@class='vip-title clearfix']/div[@class='price']/span[@class='value']/span[@class='amount']/text()").extract_first()
                prix_final=""
                pr=response.xpath("//div[@class='price']/span/span[@class='value']/span[@class='ad-price']/text()").extract_first().strip()
                if pr != []:
    ...
    #        print(province)
           ...
           #     telephone=""
            ...
            try:
               sp=response.xpath("//body/script/text()").extract_first()
               if re.findall('"sllr_nm":"(.*?)"',str(sp)) !=[]:
                item["GARAGE_NAME"]=self.correct(re.findall('"sllr_nm":"(.*?)"',str(sp))[0])
            except:
               pass
            print("#########################################",telephone)
            #print (item)
